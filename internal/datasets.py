...
class Dataset(threading.Thread):
  ...
  def _generate_rays(self,config):
    ...
    temperature = np.tile(self.temperature[:,None,None,:],[1,*ones.shape[1:3],1])
    exposure = np.tile(self.exposure[:,None,None,:],[1,*ones.shape[1:3],1])
    coc = np.tile(self.coc[:,None,None,:],[1,*ones.shape[1:3],1])
...
class LLFF(Dataset):
  ...
  def _load_renderings(self, config):
    ...
    temperature = np.load(path.join(self.data_dir, 'temperature.npy'))
    exposure = np.load(path.join(self.data_dir, 'exposure.npy'))
    coc = np.load(path.join(self.data_dir, 'coc.npy'))
    # coc = np.ones_like(exposure)
    temperature = temperature.astype(np.float32) /10000
    exposure = (exposure.astype(np.float32) +3) /6
    coc = coc.astype(np.float32)
    temperature = temperature.reshape(images.shape[3],-1)
    exposure = exposure.reshape(images.shape[3],-1)
    coc = coc.reshape(images.shape[3],-1)
    ...
    if not config.spherify and self.split == 'test':
      self._generate_spiral_poses(poses, bds,config.render_temp,config.render_exp)
...
#     i_test = np.arange(0, 12, 1)
  ...
  def _generate_rays(self,config):
    ...
    if self.split == 'test' :
      ...
      self.camtoworlds = np.concatenate([self.render_poses, self.camtoworlds],
                                        axis=0)
      self.temperature = np.concatenate([self.render_temp, self.temperature],
                                        axis=0)
      self.exposure = np.concatenate([self.render_exp, self.exposure],
                                        axis=0)
      zeros = np.ones_like(self.render_exp)
      self.coc = np.concatenate([zeros, self.coc],
                                        axis=0)
    ...
    if not self.spherify:
      ...
      temperature = self.rays.temperature
      exposure = self.rays.exposure
      coc = self.rays.coc
      self.rays = utils.Rays(
          origins=ndc_origins,
    ...
    if self.split == 'test':
      ...
      self.temperature = self.temperature[n_render_poses:]
      self.exposure = self.exposure[n_render_poses:]
  ...
  def _generate_spiral_poses(self, poses, bds, temp, exp):
    ...
    render_t = np.linspace(temp/10000,temp/10000,n_views)
    # render_t = np.linspace(5200.0 / 10000, 2000.0 / 10000, n_views)
    render_t = render_t.reshape(-1,1)
    render_t = np.array(render_t).astype(np.float32)
    render_e = np.linspace((exp+3)/6,(exp+3)/6,n_views)
    # render_e = np.linspace((-2 + 3) / 6, (1 + 3) / 6, n_views)
    render_e = render_e.reshape(-1,1)
    render_e = np.array(render_e).astype(np.float32)
    self.render_temp = render_t
    self.render_exp = render_e
