...
flags.DEFINE_integer('render_every', 5000,#5000
...
def main(unused_argv):
  ...
  dataset = datasets.get_dataset('train', config.data_dir, config) #data/horns
  # dataset_r3 = datasets.get_dataset('train', os.path.join(config.data_dir,'focus','3'), config)
  # dataset_r7 = datasets.get_dataset('train', os.path.join(config.data_dir,'focus','7'), config)
  # dataset_r15 = datasets.get_dataset('train', os.path.join(config.data_dir,'focus','15'), config)
  # dataset_r31 = datasets.get_dataset('train', os.path.join(config.data_dir,'focus','31'), config)
  # dataset_r51 = datasets.get_dataset('train', os.path.join(config.data_dir,'focus','51'), config)
  # dataset_r71 = datasets.get_dataset('train', 'data/horns_Guassblur/71', config)
  # dataset_r101 = datasets.get_dataset('train', 'data/horns_Guassblur/101', config)
  test_dataset = datasets.get_dataset('test', config.data_dir, config) #data/horns
...
# next(test_dataset)pdataset, pdataset_r11, pdataset_r21, pdataset_r51, pdataset_r101
  ...
  for step in range(init_step, config.max_steps + 1):
    ...
    # train_coc = np.random.choice([1,3,7,15,31,51],p=[0.4,0.15,0.15,0.1,0.1,0.1])
    # train_coc = np.random.choice([1, 3, 7, 15, 31], p=[0.4, 0.2, 0.2, 0.1, 0.1])
    train_coc = np.random.choice([1], p=[1])
    # train_coc = np.random.choice([1,3], p=[0.6,0.4])
    if train_coc == 1:
        batch = next(pdataset)
        train_coc = batch['rays'].coc[0]
    # elif train_coc == 3:
    ...
    if FLAGS.render_every > 0 and step % FLAGS.render_every == 0:
      ...
      utils.save_img_uint8(
          pred_color, os.path.join(data_out_path, 'color_{}.png'.format(step)))
      vis_suite = vis.visualize_suite(pred_distance, pred_acc)
      for k, v in vis_suite.items():
          utils.save_img_uint8(v, os.path.join(data_out_path, k + '_{:03d}.png'.format(step)))
      ...
      if jax.host_id() == 0:
        ...
        print('step: ' + str(step) + ' ' + 'psnr: ' + str(psnr) + '\r\n')
