...
def main(unused_argv):
  ...
  out_dir = path.join(FLAGS.train_dir,
                      'path_renders_T{}_E{}_A{}'.format(config.render_temp,config.render_exp,a1)
                      if config.render_path else 'test_preds_T{}_E{}_A{}'.format(config.render_temp,config.render_exp,a1))
  ...
  while True:
    ...
    rgbs = []
    rays = []
    n_frames = dataset.render_poses.shape[0]
    # n_frames = 20
    if not FLAGS.eval_once:
    ...
    for i in range(n_frames):
        ...
        o = dataset.render_rays.origins[i]
        d = dataset.render_rays.directions[i]
        v = dataset.render_rays.viewdirs[i]
        r = dataset.render_rays.radii[i]
        temp = dataset.render_rays.temperature[i]
        exp = dataset.render_rays.exposure[i]
        coc =dataset.render_rays.coc[i]
